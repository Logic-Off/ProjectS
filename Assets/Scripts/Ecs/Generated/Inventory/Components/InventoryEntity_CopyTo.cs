...
public partial class InventoryEntity
{
	...
	/// </summary>
	public void CopyTo(InventoryEntity copyToEntity)	{
		for (var i = 0; i < InventoryComponentsLookup.TotalComponents; ++i) {
			if (HasComponent(i)) {
				if (copyToEntity.HasComponent(i)) {
					throw new EntityAlreadyHasComponentException(
						i,
						"Cannot copy component '" +
						InventoryComponentsLookup.ComponentNames[i] +
						"' to " +
						this +
						"!",
	...
	/// </summary>
	public void CopyTo(InventoryEntity copyToEntity, bool replaceExisting) {
		for (var i = 0; i < InventoryComponentsLookup.TotalComponents; ++i) {
			if (!HasComponent(i)) {
				continue;
			}
	...
	/// <summary>
	/// Copies components on this entity at <paramref name="indices"/> in the <see cref="InventoryComponentsLookup"/> to
	...
	/// </summary>
	public void CopyTo(InventoryEntity copyToEntity, bool replaceExisting, params int[] indices) {
		for (var i = 0; i < indices.Length; ++i) {
			var index = indices[i];

			// Validate that the index is within range of the component lookup
			if (index < 0 && index >= InventoryComponentsLookup.TotalComponents) {
				...
				throw new IndexOutOfLookupRangeException(string.Format(OUT_OF_RANGE_WARNING, index, nameof(InventoryComponentsLookup)), HINT);
			}
