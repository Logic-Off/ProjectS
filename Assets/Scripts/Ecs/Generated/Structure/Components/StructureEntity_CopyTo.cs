...
public partial class StructureEntity
{
	...
	/// </summary>
	public void CopyTo(StructureEntity copyToEntity)	{
		for (var i = 0; i < StructureComponentsLookup.TotalComponents; ++i) {
			if (HasComponent(i)) {
				if (copyToEntity.HasComponent(i)) {
					throw new EntityAlreadyHasComponentException(
						i,
						"Cannot copy component '" +
						StructureComponentsLookup.ComponentNames[i] +
						"' to " +
						this +
						"!",
	...
	/// </summary>
	public void CopyTo(StructureEntity copyToEntity, bool replaceExisting) {
		for (var i = 0; i < StructureComponentsLookup.TotalComponents; ++i) {
			if (!HasComponent(i)) {
				continue;
			}
	...
	/// <summary>
	/// Copies components on this entity at <paramref name="indices"/> in the <see cref="StructureComponentsLookup"/> to
	...
	/// </summary>
	public void CopyTo(StructureEntity copyToEntity, bool replaceExisting, params int[] indices) {
		for (var i = 0; i < indices.Length; ++i) {
			var index = indices[i];

			// Validate that the index is within range of the component lookup
			if (index < 0 && index >= StructureComponentsLookup.TotalComponents) {
				...
				throw new IndexOutOfLookupRangeException(string.Format(OUT_OF_RANGE_WARNING, index, nameof(StructureComponentsLookup)), HINT);
			}
