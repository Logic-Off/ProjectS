	...
	public partial class InstallerPrefabGenerator {
		...
		public static void Generate() {
			...
			foreach (var file in _files)
				SaveToFile(file.Value, $"{file.Key}PrefabsInstaller.cs", path);
			Debug.Log("End generate prefabs installers");
		...
		private static Dictionary<string, List<Entry>> Find() {
			...
			foreach (var type in assembly.GetTypes()) {
				var attribute = type.GetCustomAttribute<InstallPrefabAttribute>();
				if (attribute == null)
					continue;
		...
		public sealed class Entry {
			public InstallPrefabAttribute Attribute;
			public Type Type;

			public Entry(Type type, InstallPrefabAttribute attribute) {
				Type = type;
				Attribute = attribute;
			}
		}
