	...
	public class AnimationUIInspector : Editor {
		public override void OnInspectorGUI() {
			...
			EditorGUI.ProgressBar(
				position,
				animationUI.CurrentTime / animationUI.TotalDuration,
				...
				if (sequence.SequenceType == SequenceType.Animation) {
					if (sequence.TargetComp != null) {
						...
						if (sequence.TargetType == SequenceObjectType.Automatic) {
							...
							if (sequence.TargetComp.GetComponent<Image>() != null)
								sequence.AtTime += " [Image]";
							else {
								sequence.TargetComp = null;
								// sequence.AtTime += " [Unassigned] [Image]";
							}
						} else if (sequence.TargetType == SequenceObjectType.Transform) {
							if (sequence.TargetComp.transform != null)
								sequence.AtTime += " [Transform]";
							else {
								sequence.TargetComp = null;
							}
							...
							if (sequence.TargetComp.GetComponent<Camera>() != null)
								sequence.AtTime += " [Camera]";
							else {
								sequence.TargetComp = null;
							}
					...
					sequence.Duration = 0;
					if ((sequence.SFXFile != null) && (sequence.PlaySFXBy == SequenceSFXMethod.File))
						sequence.AtTime += " [" + sequence.SFXFile.name + "] [SFX]";
					else if ((sequence.SFXFile == null) && (sequence.PlaySFXBy == SequenceSFXMethod.File))
