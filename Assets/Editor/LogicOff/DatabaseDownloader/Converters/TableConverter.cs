	...
	public static class TableConverter {
		...
		private static void CollectItems(
			...
			if (entry.IsArray) {
				...
				else if (header.Key.Length != 0 && columns.Count > header.Key[0])
					OnDefaultValue(builder, entry.Name, columns[header.Key[0]]);
				else {
					// Если всё что выше не проходит, тогда надо заполнить пустыми данными
					OnDefaultValue(builder, entry.Name, string.Empty);
				}
			}
		}
		...
		private static void OnVerticalArray(
			...
			for (var index = currentIndex; index < tableRows.Count; index++) {
				...
				if (tableRows.Count <= index || header.Key.Length == 0 || tableRows[index].Count <= header.Key[0])
					continue;
		...
		private static bool TryCollectValues(
			...
			foreach (var pair in elements) {
				i++;
				if (tableRows.Count <= currentIndex || pair.Key.Length == 0 || tableRows[currentIndex].Count <= pair.Key[0])
					continue;
		...
		private static string ConvertValue(string value) {
			var isNumeric = double.TryParse(value, NumberStyles.Any, CultureInfo.InvariantCulture, out var number);
			// InvariantCulture заменяет запятую на точку
			if (!isNumeric)
				value = value.Replace("\"", "\\" + "\"");
			return isNumeric ? number.ToString().Replace(',', '.') : $"\"{value}\"";
		}
	}
}